...
void LightShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, Light* light, Light* light2)
{
	HRESULT result;
	...
	XMFLOAT4 positon1 = { p.x, p.y, p.z, 1 };
	p = light2->GetPosition();
	XMFLOAT4 positon2 = { p.x, p.y, p.z, 1 };
	...
	lightPtr->position[0] = positon1;
	lightPtr->diffuse[1] = light2->GetDiffuseColour();
	lightPtr->position[1] = positon2;
