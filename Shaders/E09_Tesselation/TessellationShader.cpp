...
void TessellationShader::InitShader(WCHAR* vsFilename,  WCHAR* psFilename)
{
	...
	//tess buffer
	// Setup the description of the dynamic matrix constant buffer that is in the vertex shader.
	tessBufferDesc = ReturnBasicBufferDesc(sizeof(TessBufferType));

	m_device->CreateBuffer(&matrixBufferDesc, NULL, &m_tessBuffer);
...
void TessellationShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, float tess_factor)
{
	...
	//tess buffer
	result = deviceContext->Map(m_tessBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);

	// Get a pointer to the data in the constant buffer.
	tessPtr = (TessBufferType*)mappedResource.pData;


	tessPtr->tess_factor = tess_factor;

	// Unlock the constant buffer.
	deviceContext->Unmap(m_tessBuffer, 0);

	// Set the position of the constant buffer in the vertex shader.
	bufferNumber = 0;

	// Now set the constant buffer in the vertex shader with the updated values.
	deviceContext->HSSetConstantBuffers(bufferNumber, 1, &m_tessBuffer);
